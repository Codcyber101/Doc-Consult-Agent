...
export default function DocumentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/10 selection:text-foreground font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            ...
            <Button variant="outline" className="rounded-xl border-border text-danger hover:bg-red-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-8">
            <Card className="overflow-hidden border-border shadow-xl relative aspect-[16/10] bg-foreground group">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="flex flex-col items-center gap-4 text-surface/20">
                  <FileText className="w-24 h-24" />
                  <p className="font-display font-bold text-2xl uppercase tracking-tighter">Digital Archive Preview</p>
                </div>
              </div>

              <div className="absolute inset-12 border-2 border-primary/20 rounded-2xl flex flex-col justify-end p-10 bg-gradient-to-t from-black/80 to-transparent">
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-[10px] font-black text-accent uppercase tracking-widest mb-1">Authenticated Holder</p>
                    <h2 className="text-3xl font-display font-bold text-surface tracking-tight">{doc.extraction[0].value}</h2>
                    <p className="text-sm text-surface/70 font-mono mt-2">{doc.id}</p>
                  </div>
                  <div className="w-24 h-24 bg-surface/5 rounded-xl border border-surface/10 flex items-center justify-center">
                    <Database className="w-8 h-8 text-surface/20" />
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-display font-bold text-foreground">Agent Intelligence</h3>
                <span className="text-[10px] font-black uppercase tracking-widest text-muted">Validated Fields</span>
              </div>
                ...
                {doc.extraction.map((item: any, i: number) => (
                  <Card key={i} className="bg-surface border-border">
                    <CardContent className="p-5 flex justify-between items-center">
                      <div>
                        <p className="text-[10px] font-black text-muted uppercase tracking-widest mb-1">{item.field}</p>
          ...
          <div className="lg:col-span-4 space-y-8">
              ...
              <CardContent className="p-6 space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-3 p-4 bg-primary/10 border border-primary/20 rounded-2xl">
                    ...
                    <div>
                      <p className="text-xs font-bold text-foreground">Cryptographically Signed</p>
                      <p className="text-[10px] text-primary uppercase font-black">HSM National Root</p>
                    </div>
                  </div>

                  <Button variant="sovereign" className="w-full h-12 rounded-xl text-xs gap-2" onClick={() => setIsProvenanceOpen(true)}>
                ...
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-muted">Sharing History</h4>
                  <div className="space-y-4">
                    {doc.history.map((log: any, i: number) => (
                      <div key={i} className="flex gap-4">
                        <div className="w-1 h-10 bg-surface-muted rounded-full shrink-0" />
                        <div>
                          <p className="text-xs font-bold text-foreground">{log.agency}</p>
                          <p className="text-[10px] text-muted uppercase">{log.action} • {log.date}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <Button variant="outline" className="w-full h-14 rounded-2xl border-border gap-2 font-bold text-muted">
            ...
            <div className="p-6 bg-foreground rounded-[2rem] text-surface space-y-4 relative overflow-hidden">
      ...
      <ProvenanceViewer isOpen={isProvenanceOpen} onClose={() => setIsProvenanceOpen(false)} data={doc.provenance} />
    </div>
  );
}
