...
export default function VaultPage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background selection:bg-primary/10 selection:text-foreground font-sans">
      <div className="fixed inset-0 bg-mesh pointer-events-none opacity-30" />
      <AppSidebar />

      <main className={cn(
        "flex-1 flex flex-col min-w-0 transition-all duration-300 relative z-10 h-screen overflow-y-auto",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-primary">{t('vault.encryptedStorage')}</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-display font-bold text-foreground tracking-tight">
                {t('vault.title')} <span className="text-primary italic">{t('vault.titleAccent')}</span>
              </h1>
              <p className="mt-4 text-lg text-muted">{t('vault.subtitle')}</p>
            </div>
            ...
            <div className="bg-surface border border-border p-6 rounded-3xl flex items-center gap-6 shadow-sm">
              <div className="flex flex-col items-center">
                ...
                <span className="text-[10px] font-black text-muted uppercase">{t('vault.documentsCount')}</span>
              </div>
              <div className="w-px h-10 bg-surface-muted" />
              <div className="flex flex-col items-center">
                <span className="text-2xl font-bold text-primary">100%</span>
                <span className="text-[10px] font-black text-muted uppercase">{t('vault.verified')}</span>
              </div>
              <Button className="ml-4 bg-foreground hover:opacity-90 text-surface rounded-xl">{t('vault.addNew')}</Button>
            </div>
          </div>
                  ...
                  <Card className="group overflow-hidden hover:border-primary/50 transition-all duration-300">
                    <div className="aspect-[4/3] bg-surface-muted relative overflow-hidden flex items-center justify-center border-b border-border">
                      ...
                      <div className="absolute bottom-4 left-4 right-4 flex gap-2 justify-center">
                        ...
                        >
                          <Eye className="w-4 h-4 mr-2" /> {t('vault.view')}
                        </Button>
                        <Button
                          size="sm"
                        ...
                        >
                          <ShieldCheck className="w-4 h-4 mr-2 text-accent" /> {t('vault.verify')}
                        </Button>
                      </div>
                    </div>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="space-y-3 pt-4 border-t border-border">
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-muted font-medium uppercase tracking-wider">{t('vault.issuedBy')}</span>
                          <span className="text-foreground font-bold">{doc.issuedBy}</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-muted font-medium uppercase tracking-wider">{t('vault.expires')}</span>
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="mt-16 bg-foreground rounded-[2.5rem] p-10 text-surface relative overflow-hidden">
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
              ...
              <div>
                <h3 className="text-2xl font-display font-bold mb-2 tracking-tight">{t('vault.encryptionNoticeTitle')}</h3>
                <p className="text-surface/70 text-lg leading-relaxed max-w-2xl">
                  {t('vault.encryptionNoticeDesc')}
                </p>
              </div>
              <Button variant="secondary" className="ml-auto px-8 h-14 rounded-2xl font-bold">
                {t('vault.securityAudit')}
              </Button>
            </div>
