       ...
       <CardHeader className="pb-2 border-b border-black/5">
         <CardTitle className="flex items-center justify-between font-display text-ink">
            ...
            <span className={cn(
              ...
              isReady 
                ? "bg-emerald-50 text-emerald-700 border-emerald-700/20" 
             ...
             <div className="relative flex-shrink-0 w-36 h-36 flex items-center justify-center bg-white rounded-full shadow-inner border border-slate-100">
                 <svg className="w-28 h-28 transform -rotate-90">
                    ...
                    <motion.circle
                      ...
                      className={cn(
                        score < 50 ? "text-red-500" : score < 100 ? "text-gold-500" : "text-emerald-600"
                      )}
                    />
                 </svg>
                 <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-4xl font-black font-display text-ink tracking-tighter">{score}%</span>
                ...
                {items.map((item) => (
                  <motion.div 
                    ...
                    className={cn(
                      ...
                      "bg-emerald-50/10 border-slate-100 hover:bg-white"
                    )}
                  >
                     <div className="flex items-center gap-3">
                        ...
                        <span className={cn(
                          "text-sm font-medium",
                          item.status === 'ready' ? "text-slate-500 line-through decoration-slate-300" : "text-ink"
                        )}>
                          {item.label}
                        </span>
                     </div>
                     ...
                     {item.status !== 'ready' && (
                        <Button 
                          ...
                          className={cn(
                             ...
                             item.status === 'missing' ? "bg-red-earth hover:bg-red-600" : "bg-gold-400 hover:bg-gold-500 text-black"
                          )}
