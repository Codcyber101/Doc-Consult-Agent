...
class ProceduralGuideAgent:
    ...
    def get_playbook(
        self, jurisdiction: str, process_id: str
    ...
    def get_next_step(
        self, playbook: Dict[str, Any], completed_steps: List[str]
    ) -> Dict[str, Any]:
        """
    ...
    def evaluate_readiness(
        self, playbook: Dict[str, Any], extracted_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        score = (
            (len(required_docs) - len(missing)) / len(required_docs)
            if required_docs
            else 1.0
        )
