...
class RAGSummarizer:
    ...
    async def summarize(self, query: str, snippets: List[Dict]) -> Dict:
        ...
        context_text = "\n\n".join(
            [f"Source [{i}]: {s['content']}" for i, s in enumerate(snippets)]
        )
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are an expert legal assistant for GovAssist Ethiopia. "
        ...
        confidence_score = (
            0.9
            if len(verified_citations) == len(citations) and len(citations) > 0
            else 0.5
        )
