...
from regulation_expert.summarization import RAGSummarizer
from common.safety import SafetyAgent
from common.citation import CitationAuditor
...
async def test_rag_summarization_flow(rag_summarizer):
    ...
    with patch.object(
        rag_summarizer.llm, "_agenerate", AsyncMock(return_value=mock_result)
    ):
...
async def test_rag_pii_masking_in_summarization(rag_summarizer):
    ...
    snippets = [
        {"id": "s1", "content": "Taxes are mandatory.", "document_id": "PROC-1"}
    ]
